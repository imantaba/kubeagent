...
6. `--wait-for` scope: only findings attributable to that workload decide the exit code; everything else is printed as not counted. Accepted kinds: `deployment`, `statefulset`, `daemonset`.
