...
The `--kubeconfig` value is deliberately a path that cannot exist, so that if validation ever moved after `cluster.NewClient` the message assertion fails instead of the test passing by accident on a coincidentally matching exit code.

**No failure path may exit 1 without a verdict.** Exit 1 is a statement — kubeagent saw the cluster and found problems at or above `--fail-on`. A bare `return err` from `runGate` lands on `exitCodeFor`'s default of 1, which would make an unreadable kubeconfig indistinguishable from a cluster full of CrashLoopBackOff. Add this test to `main_test.go`; it is the regression guard for all three error paths:
...
Add `"strings"` to `main_test.go`'s imports if not already there. The `scan.Evaluate` and `rolloutwait.Wait` failure paths take `gate.CodeInconclusive` for the matching reason — they reached the cluster and could not see it — but both need a live cluster to exercise, so they are covered by the release gate's blind-service-account run rather than here.
