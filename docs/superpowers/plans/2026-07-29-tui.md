...
**Layout the golden frame must produce at 80×24** (blank padding rows between
the last finding and the lower rule elided here; the two rules are 80
box-drawing characters, the full terminal width):
...
▸ critical Pod          shop        crasher-7d9f-x2    CrashLoopBackOff
  ...
  warning  Deployment   shop        api                not fully available
────────────────────────────────────────────────────────────────────────────────
[1] critical [2] warning+ [0] all (3)  ↑↓ move  ⏎ detail  ? help  q quit
```

The ISSUE column carries `Finding.Issue`, not `Finding.Reason` — the warning row
reads `not fully available`, and its `1/3 ready` reason belongs to the detail
view. The footer has shed `b blind` and `r rescan` at this width: the full key
map is 91 columns, `? help  q quit` is pinned, and the blind-spot count in the
header already hints `b`.
...
//
// reflect.DeepEqual, not ==: scan.Options holds ExpectedNodes []string, so the
// struct is not comparable and == does not compile. This still compares the
// whole value — which is the point of the test — rather than a field at a time,
// so an option added to gateScanOptions and forgotten here still fails.
...
`main_test.go` needs `reflect` in its import block if it is not already there.
