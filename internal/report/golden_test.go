...
func goldenInput(now time.Time) Input {
	return Input{
		...
		WebhookIssues: []webhookhealth.Issue{
			...
			{Kind: "ValidatingWebhookConfiguration", Config: "slow-validator", Webhook: "policy.example.com", Problem: "HighTimeout",
...
func goldenWorkloads() []inventory.Workload {
	...
	return []inventory.Workload{
		{Namespace: "shop", Name: "web", Kind: "Deployment", Desired: 1, Ready: 0, Status: "Degraded",
			...
			Pods:     []inventory.PodRow{{Name: "web-5b8-2wplt", Phase: "Running", State: "CrashLoopBackOff", Ready: "0/1", Restarts: 8, LastRestart: r, Node: "worker-1", IP: "10.244.2.2", Age: "20d", Image: "busybox:1.36"}},
		...
		{Namespace: "shop", Name: "api", Kind: "Deployment", Desired: 2, Ready: 0, Status: "Degraded",
			...
			Pods:     []inventory.PodRow{{Name: "api-864-dxtdh", Phase: "Pending", State: "ImagePullBackOff", Ready: "0/1", Restarts: 0, Node: "worker-1", IP: "10.244.2.4", Age: "6d", Image: "nginx:9.9.9-nope"}},
		...
		{Namespace: "shop", Name: "frontend", Kind: "Deployment", Desired: 1, Ready: 0, Status: "Degraded",
			...
			Pods:     []inventory.PodRow{{Name: "frontend-58d-x2vqp", Phase: "Pending", State: "ImagePullBackOff", Ready: "0/1", Restarts: 0, Node: "worker-3", IP: "10.244.3.14", Age: "3h", Image: "ghcr.io/shop/frontend:2.4"}},
		...
		{Namespace: "shop", Name: "search", Kind: "Deployment", Desired: 1, Ready: 0, Status: "Degraded",
			...
			Pods:     []inventory.PodRow{{Name: "search-7b4-mm1zq", Phase: "Pending", State: "ErrImagePull", Ready: "0/1", Restarts: 0, Node: "worker-3", IP: "10.244.3.15", Age: "3h", Image: "ghcr.io/shop/search:1.9"}},
		...
		{Namespace: "shop", Name: "reports", Kind: "Deployment", Desired: 1, Ready: 0, Status: "Pending",
			...
			Pods: []inventory.PodRow{{Name: "reports-6c9-fk2vw", Phase: "Pending", State: "Pending", Ready: "0/1", Restarts: 0, Node: "worker-3", IP: "", Age: "2h", Image: "busybox:1.36"}}},
		{Namespace: "shop", Name: "billing-worker", Kind: "Deployment", Desired: 1, Ready: 0, Status: "Degraded",
			...
			Pods: []inventory.PodRow{{Name: "billing-7c7-vbgd7", Phase: "Running", State: "CrashLoopBackOff", Ready: "0/1", Restarts: 6, LastRestart: r, Node: "worker-2", IP: "10.244.1.2", Age: "18d", Image: "polinux/stress"}},
		...
		{Namespace: "shop", Name: "data", Kind: "StatefulSet", Desired: 1, Ready: 0, Status: "Degraded",
			...
			Pods:     []inventory.PodRow{{Name: "data-0", Phase: "Pending", State: "ContainerCreating", Ready: "0/1", Restarts: 0, Node: "worker-2", IP: "", Age: "9d", Image: "postgres:16"}},
		...
		{Namespace: "shop", Name: "checkout", Kind: "Deployment", Desired: 1, Ready: 0, Status: "Degraded",
			Image: "checkout:2.1",
			Pods:  []inventory.PodRow{{Name: "checkout-7f9-qk2mn", Phase: "Running", State: "Running", Ready: "0/1", Restarts: 0, Node: "worker-3", IP: "10.244.3.9", Age: "5d", Image: "checkout:2.1"}},
		...
		// This row used to carry Phase: "Init:CrashLoopBackOff", a value
		// inventory.PodRowFor cannot produce: it assigns string(p.Status.Phase),
		// and that field only ever holds one of five phase words. The fixture had
		// been supplying the kubectl-style word the report wanted from a field
		// that never carries one. Phase is the phase a pod with a crash-looping
		// init container really has, and State is where the word belongs.
		{Namespace: "shop", Name: "orders", Kind: "Deployment", Desired: 1, Ready: 0, Status: "Degraded",
			Image: "orders:3.0",
			Pods:  []inventory.PodRow{{Name: "orders-6f9-qk2mn", Phase: "Pending", State: "Init:CrashLoopBackOff", Ready: "0/1", Restarts: 0, Node: "worker-3", IP: "10.244.3.11", Age: "4m", Image: "orders:3.0"}},
		...
		// "Last run failed", not "Idle": batchhealth.Annotate rewrites the status on
		// the same branch that attaches this finding, so a CronJob carrying one can
		// no longer reach the renderer saying "Idle".
		{Namespace: "shop", Name: "nightly-report", Kind: "CronJob", Desired: 0, Ready: 0, Status: "Last run failed", Schedule: "0 2 * * *",
